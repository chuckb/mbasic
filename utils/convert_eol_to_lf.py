#!/usr/bin/env python3
"""
Convert all BASIC files to Unix-style LF line endings.

Rules:
1. Files with CRLF (\r\n) -> convert to LF (\n)
2. Files with only CR (\r) -> convert to LF (\n)
3. Files already with LF (\n) -> leave as is
"""
...
def main():
    ...
    if not basic_dir.exists():
        print(f"ERROR: {basic_dir} does not exist")
        return 1

    # Find all .bas and .BAS files
    files = list(basic_dir.rglob("*.bas")) + list(basic_dir.rglob("*.BAS"))

    print(f"Found {len(files)} BASIC files")
    print()
    ...
    for file_path in sorted(files):
        ...
        if changed:
            rel_path = file_path.relative_to(basic_dir)
